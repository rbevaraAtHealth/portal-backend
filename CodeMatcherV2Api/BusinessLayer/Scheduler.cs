...
namespace CodeMatcher.Api.V2.BusinessLayer
{
    public class Scheduler : IScheduler
    {
        ...
        {
            var schedulerRecords = await _context.CodeMappingRequests.AsNoTracking().Include("RunType").Include("SegmentType").Include("CodeMappingType").Where(x => x.RunType.Name.Equals("Scheduled")).OrderByDescending(x => x.CreatedTime).ToListAsync();
